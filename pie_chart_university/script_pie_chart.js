...
// Load external data 
var promises = [
  d3.csv("./uni_counter.csv", function(csv_data) {
    // console.log("asd")
    // for (var i = 0; i < csv_data.length; i++) {
      var temp = {};
      temp["id"] = csv_data.id;
      temp["label"] = csv_data.label;
      temp["perc"] = Number(csv_data.perc);
      temp["color"] = csv_data.color;
      data.push(temp);
    // }
    console.log("asd1", data)
  })
];
...
function ready(){
  ...
  */
  ...
  .html(({ data: d }) => `
    <tspan x="0" dy="0" font-size="7.5">${d.label}:</tspan>${Number(d.perc)}% </tspan>
  `)
  .style('opacity', 0)
